...
class MLPlay:
    ...
    def update(self, scene_info, keyboard=[], *args, **kwargs):
        ...
        if scene_info["status"] != "GAME_ALIVE":
            ...
            self.pickle_file = f"data/bad_data/slide_{self.slide}/pl/level_{level}/game_data_{int(time.time())}.pkl"
            if level < 10:
                self.pickle_file = f"data/bad_data/slide_{self.slide}/level_0{level}/game_data_{int(time.time())}.pkl"
        ...
        if(scene_info["frame"]%100==1) :
            level=abs(scene_info["ball_speed"][1])
            self.pickle_file = f"data/good_data/slide_{self.slide}/level_{level}/game_data_{int(time.time())}.pkl"
            if level < 10:
                self.pickle_file = f"data/good_data/slide_{self.slide}/level_0{level}/game_data_{int(time.time())}.pkl"
            if not os.path.exists(os.path.dirname(self.pickle_file)):
                os.makedirs(os.path.dirname(self.pickle_file), exist_ok=True)
            with open(self.pickle_file, mode="ab") as file:
                pickle.dump(self.data_buffer, file)
        ...
        if self.side == "1P":
            ...
            future[1]=prediction.predict(self.mindow[0],(self.mindow[1][1]+3,self.mindow[1][1]),self.mindow[2],self.mindow[3],self.mindow[4],0,"UP",self.slide)#正
